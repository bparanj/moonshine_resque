...
These tasks assume you've added the `:resque` role to the servers using Resque.

You may even go so far as to add a hook to have your Resque workers restart on
deploys:

    before 'god:restart', 'resque:stop'

This is a great way to handle automatically increasing or decreasing the
